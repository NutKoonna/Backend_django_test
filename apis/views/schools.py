...
class StudentSubjectsScoreAPIView(APIView):
    ...
    def post(request, *args, **kwargs):
        ...
        if (student_first_name == None or student_last_name == None or subjects_title == None or score == None):
            return Response(status=status.HTTP_400_BAD_REQUEST)

        if not (isinstance(score, (float, int)) and 0 <= score <= 100):
            raise BadRequest("Score must be number, equal or greater than 0 and equal or less than 100")
        personnel_object = Personnel.objects.filter(first_name=student_first_name, last_name=student_last_name).first()
        subject_object = Subjects.objects.filter(title=subjects_title).first()

        if personnel_object == None or subject_object == None:
            return Response(status=status.HTTP_400_BAD_REQUEST)
        student_subject_score_object = StudentSubjectsScore.objects.filter(student=personnel_object.pk,
                                                                           subjects=subject_object.pk).first()

        if not (isinstance(student_subject_score_object.credit, int) and 0 < student_subject_score_object.credit <= 3):
            raise BadRequest("Credit must be integer, greater than 0 and equal or less than 3.")
        if not (isinstance(student_first_name, str) and isinstance(student_last_name, str)
                and isinstance(subjects_title, str) and isinstance(score, (float, int))):
            return Response(status=status.HTTP_400_BAD_REQUEST)

        student_subject_score_object.score = score
        student_subject_score_object.save()
...
class StudentSubjectsScoreDetailsAPIView(APIView):
    ...
    def get(request, *args, **kwargs):
        ...
        student_subject_score_filter = StudentSubjectsScore.objects.filter(student=student_id)
        if len(student_subject_score_filter) == 0:
            return Response('Score not found', status=status.HTTP_400_BAD_REQUEST)

        subject_detail = []
        personel_filter = Personnel.objects.filter(pk=student_id).first()
        # if personel_filter.score ==:
        #    return Response('Id not found', status=status.HTTP_400_BAD_REQUEST)
        grade_point = {"A": 4, "B+": 3.5, "B": 3, "C+": 2.5, "C": 2, "D+": 1.5, "D": 1, "F": 0}
        test = dict.fromkeys(["subject", "credit", "score", "grade"])
        example_context_data = {
            "student":
                {
                    "id": student_subject_score_filter,
        ...
        def map_subject_details(subject, grade):
            context = {
        ...
        for subject in student_subject_score_filter:
            score = subject.score
            if 80 <= score <= 100:
                grade = 'A'
            elif 75 <= score < 80:
                ...
                grade = 'F'
            subject_detail.append(map_subject_details(subject, grade))

        example_context_data["student"]["id"] = student_subject_score_filter[0].student.pk
        example_context_data["student"]["full_name"] = student_subject_score_filter[0].student.first_name + " " + \
                                                       student_subject_score_filter[0].student.last_name
        example_context_data["student"]["school"] = student_subject_score_filter[0].student.school_class.school.title
        example_context_data["subject_detail"] = subject_detail
        cal_grade_point = 0
        for data in example_context_data["subject_detail"]:
            cal_grade_point += data.get('credit') * (grade_point[data.get('grade')])
        grade_point_average = cal_grade_point / len(example_context_data["subject_detail"])
        example_context_data["grade_point_average"] = grade_point_average
...
class PersonnelDetailsAPIView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if school_title == None:
            return Response('school titile not found', status=status.HTTP_400_BAD_REQUEST)
        class_order = []
        your_result = []
        count_order = 1
        schools_filter = Schools.objects.filter(title=school_title).first()
        classes_filter = Classes.objects.filter(school=schools_filter.pk)
        for order in classes_filter:
            class_order.append(order.pk)
        personnel_filter = Personnel.objects.filter(school_class__in=class_order).order_by('personnel_type',
                                                                                           'school_class__class_order',
                                                                                           'first_name')
        for personnel in personnel_filter:
            personnel.first_name.capitalize()
            personnel.last_name.capitalize()
            personnel.school_class.school.title.capitalize()
            if personnel.personnel_type == 0:
                personnel.personnel_type = 'Teacher'

            if personnel.personnel_type == 1:
                personnel.personnel_type = 'Head of the room'
            if personnel.personnel_type == 2:
                personnel.personnel_type = 'Student'

            your_result.append(
                f'{count_order}. school:{personnel.school_class.school.title}, role: {personnel.personnel_type}, class: {personnel.school_class.class_order}, name: {personnel.first_name + " " + personnel.last_name}')
            count_order += 1
...
class SchoolHierarchyAPIView(APIView):
    ...
    def get(request, *args, **kwargs):
        ...
        for school in Schools.objects.all():
            ...
            for class_obj in school.classes_set.all():
                class_data = {
                    f"class {class_obj.class_order}": {}
                }
                personnel_data = Personnel.objects.select_related('school_class__school').filter(
                    school_class=class_obj
                )

                for person in personnel_data:
                    if person.personnel_type == 0:
                        class_info = {}
                        role = 'Teacher'
                        class_data[f"class {class_obj.class_order}"].update(
                            {f"role: {person.first_name} {person.last_name}": []})
                        teacher_name = f"role: {person.first_name} {person.last_name}"

                    if person.personnel_type == 1:
                        role = "Head of the room"
                        class_data[f"class {class_obj.class_order}"][teacher_name].append(
...
class SchoolStructureAPIView(APIView):
    ...
    def get(request, *args, **kwargs):
        ...
        def build_hierarchy(parent):
            school_struc = []
            school_struc_filter = SchoolStructure.objects.filter(parent=parent)
            for obj in school_struc_filter:
                structure = {'title': obj.title}
                sub_structure = build_hierarchy(obj.pk)
                if sub_structure:
                    structure['sub'] = sub_structure
                school_struc.append(structure)
            return school_struc

        your_result = build_hierarchy(None)
